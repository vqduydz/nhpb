...
const ReceiverUpdate = ({ updateModel, setUpdateModel, setReceiver, receiver }) => {
  ...
  const [districtSelect, setDistrictSelect] = useState(() =>
    updateModel.orderer && place?.district ? place.district : '---',
  );
  const [wardList, setWardList] = useState([]);
  const [wardSelect, setWardSelect] = useState(place?.ward || '');
  const [addressList, setAddressList] = useState([]);
  const [addressSelect, setAddressSelect] = useState(place?.address || '');
  const [position, setPosition] = useState(JSON.parse(currentUser.position));
